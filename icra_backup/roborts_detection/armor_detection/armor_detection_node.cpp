  ...
  while(running_) {
    ...
    if (node_state_ == NodeState::RUNNING) {
      ...
      if(detected_enemy_) {
        ...
        enemy_duration_time = enemy_duration.toSec();     //敌人丢失时间,单位s
        ROS_INFO("enemy_duration_time:%f",enemy_duration_time);
        gimbal_control_.Transform(armors[0].target_3d, pitch, yaw);
        gimbal_angle_.yaw_mode = true;          //相对运动
        gimbal_angle_.pitch_mode = false;       //绝对运动
        gimbal_angle_.pitch_angle = pitch; 
        double gimbal_yaw = GetGimbalYaw();     //获得云台当前的绝对角度
        delta_yaw = yaw;
        all_yaw = gimbal_yaw + delta_yaw;       //预期运动后的绝对角度
        ...
        gimbal_control_time = duration.toSec();     //云台控制时间
        speed = (all_yaw - last_yaw_) / gimbal_control_time;  //平均角速度
        last_yaw_ = all_yaw;
        last_time_ = ros::Time::now();              //上次云台控制时间末点
        ...
        gimbal_angle_.yaw_angle =  0.3 * yaw;     //为何0.3???
        PublishMsgs();
        std::lock_guard<std::mutex> guard(mutex_);
        ...
        last_enemy_time = ros::Time::now();     //上次敌人被识别到时的时间点
